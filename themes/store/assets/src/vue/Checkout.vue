        ...
        <form class="checkout__row" @submit.prevent="onCheckout">
          ...
          <div class="checkout__footer">
            ...
            <p class="checkout__confirm">Оформляя заказ, вы принимаете условия
             ...
             <span class="checkout__response" v-if="errorResponse">{{ errorResponse }}</span>
          </div>
        </form>
      </div>
    </div>
  ...
  methods: {
    onCheckout() {
      ...
      axios.post('/api/add-order', this.form)
        ...
        .catch(error => {
          this.loading = false;
          this.errorResponse = 'Произошла ошибка. Попробуйте другой способ оплаты или оформите заказ позже.';
